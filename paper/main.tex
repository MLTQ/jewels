...
error by 3.8\%; it is rejected under the predeclared stability gate.
...
run three autonomous windows with exact append-only identity, and isolate a lifecycle/appearance
coupling that scalar foreground weighting does not resolve. We additionally articulate two
...
\begin{itemize}
  ...
  \item \textbf{Lifecycle--appearance coupling.} Foreground-weighted supervision improves global
  fidelity and some subject edges, but the best arm raises quiet-region flicker. A shared flow trunk
  cannot guarantee lifecycle invariance merely by excluding temporal dimensions from an extra loss.
  ...
  actors, hands, and thin objects. The scalar saliency sweep improves aggregate fidelity but does
  not remove this error without a quiet-region stability regression.
...
\begin{enumerate}
  \item freeze the passing 1,024-rank empty-state plus two-continuation rollout as the deterministic
  cross-domain baseline;
  \item factor lifecycle from appearance: independently integrate the frozen base trajectory,
  copy temporal center/time covariance exactly, and train only an appearance/geometry residual;
  \item accept foreground/motion supervision only if at least three classes gain subject detail
  with bit-identical lifecycle state and no quiet-region temporal regression;
...
next falsifiable model must separate an exact frozen lifecycle trajectory from a learned
appearance/geometry residual. This hybrid preserves the distinctive explicit state while
inheriting semantic competence that the current corpus cannot teach; prompt semantics still live
in the pretrained scaffold, and direct distillation remains a later gate.
...
\begin{enumerate}
  ...
  seeds/prompts, and a lifecycle-factorized foreground-detail model without quiet-region flicker.
