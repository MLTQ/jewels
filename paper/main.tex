...
lifecycle coordinates, topology, density, and stable IDs bit-identical across 12 controls.
...
We additionally articulate two
...
\begin{itemize}
  ...
  \item \textbf{Conservative lifecycle factorization.} Exact two-stream ownership now passes the
  macro stability gate only when the residual is restricted to RGB in the top 20\% scaffold-salient
  cells. Full geometry/appearance still flickers, and two classes retain sub-\(10^{-5}\) quiet
  increases even in the selected arm. A dedicated adapter and broader gate remain open.
  ...
  actors, hands, and thin objects. The factorized RGB residual passes stability but gains only
  0.034 dB globally; it does not recover the 7.24 dB gap to the fitted ceiling.
...
\begin{enumerate}
  ...
  \item replace the second full flow copy with a zero-initialized appearance adapter over frozen v1
  features, retaining bit-identical base sampling and the same correct/shuffled/null audit;
  \item expand the adapter beyond RGB only when at least three classes gain subject detail without
  raising macro quiet-region error or weakening prompt-scaffold separation;
...
0.034 dB. The next falsifiable model is therefore a zero-initialized appearance adapter that can
expand this safe residual without relaxing ownership. The hybrid preserves the distinctive explicit
state while inheriting semantic competence that the current corpus cannot teach; prompt semantics
still live in the pretrained scaffold, and direct distillation remains a later gate.
...
\begin{enumerate}
  ...
  \item Extend the passing initial-plus-two-continuation result across longer horizons, multiple
  seeds/prompts, and a purpose-built lifecycle-factorized adapter with larger gains and no
  per-class quiet-region regression.
