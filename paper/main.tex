...
\date{Technical report -- August 14, 2026}
...
\begin{abstract}
We investigate representing ordinary RGB video as an explicit field of anisotropic Gaussian
primitives (\emph{jewels}) in normalized image-time coordinates: each primitive occupies a
volume in \uvt, carries opacity and a linear color field, and tilts through time so that motion
is oriented space-time support rather than frame-to-frame change. The goal is a promptable
text-to-video model whose generation is the \emph{composition} of such primitives --- persistent
across windows by stable identity, editable by direct manipulation, rendered deterministically
--- with semantics supplied during training by a frozen pretrained video prior (LTX-2.3) through
a coarse scaffold raster of about six kilobytes per 16-frame stride.

Four results define the current state. First, the representation itself is not the bottleneck:
optimized 72k-jewel fields replay held-out generated videos at 28--35 dB and 0.098 LPIPS ---
near-photoreal perceptual quality. Second, direct set-space generation was measured to a wall
and replaced: every mark-space sampler --- deterministic, stochastic, and two coupling
variants --- rendered worse macro-structure than a no-model trilinear decode of its own
scaffold input, across every corpus size and domain, a joint-composition failure our metric
triangulation (reference, perceptual, and texture-blind layout) localizes precisely. The
selected architecture is instead a dense intermediate: a $\sim$5M feed-forward encoder
performs amortized fitting of teacher-generated video into jewel fields and one-shots
held-out clips at 23.4 dB / 0.944 SSIM / 0.405 LPIPS with macro-layout at the fitted ceiling
--- the first arm to beat that trilinear baseline on every metric, eight decibels beyond the
best set-generative arm, from twelve training windows. Third, quality scales with
data and is priced in compute: a frozen-protocol curve over 4/8/12 fitted training sources is
monotone in the mean, with a measured three-seed noise floor resolving its first step at three
standard deviations; training on teacher-generated fits removes a cross-domain feature-loss tax
of one third at thirteen standard deviations; and a mixed real-plus-teacher corpus shows the two
supervision sources are complementary (texture versus macro-structure). One fitted source costs
about 25 GPU-minutes and the teacher supplies prompt-matched sources without limit. Fourth,
direct text selectivity remains open: correct and shuffled prompts tie once the video scaffold
is present, so prompt semantics currently live in the teacher, and distilling the teacher's
coarse scaffold into a compact text-conditioned generator is the identified route to prompt-only
inference.

Gaussian reconstruction of video is established art (VeGaS, GSVC, GaussianVideo, GaRV); the
narrower opportunity evaluated here is a corpus-level generative distribution over persistent,
editable space-time Gaussian fields. This is a progress report: negative results, falsified
hypotheses, and failed visual audits are reported as first-class evidence, and no claim is made
that current samples are competitive with raster video generation.
\end{abstract}
...
All three metrics improve monotonically in the mean, and a three-seed noise floor at the
12-source points (mark-flow training seed varied; topology and rollout seeds fixed) sorts the
claims by significance: velocity seed-sd is 0.043 (UCF) and 0.038 (domain), LPIPS sd 0.009 and
0.013. The 4-to-8-source velocity step (0.133) is about three standard deviations and stands;
the 8-to-12 step (0.050 against the multi-seed mean of 1.608) is about 1.2 and is not
individually resolved; per-step LPIPS improvements are about one standard deviation each. The
curve's tail therefore needs multi-seed subset points or larger corpus steps --- which is what
corpus scale-up provides. The economics make the axis actionable either way: one curve point
trains in under ten minutes on a consumer GPU, one fitted source costs about 25 GPU-minutes
under the frozen 72k contract, and the frozen LTX teacher supplies prompt-matched source videos
without limit ($\sim$3 minutes each). Remaining caveats: four evaluation clips, and a
correct-versus-shuffled margin that thins at twelve sources.
...
domain-matched training cuts held-out velocity loss by a third (three seeds per arm: 1.608
$\pm$ 0.043 to 1.072 $\pm$ 0.038, a thirteen-standard-deviation gap) and brings rollout LPIPS
to macro parity with the guide-upsample baseline (0.675 $\pm$ 0.013 versus 0.6761; the 0.021
advantage over the UCF arm is only $\sim$1.9 sd and is claimed as parity, not superiority),
while PSNR falls from 14.17 $\pm$ 0.02 to 12.71 $\pm$ 0.22. The visual audit, however, fails: the domain-matched contact
...
The divergence itself was then diagnosed. A layout signature (average-pooling texture away at
factor 8 before scoring) quantifies what the contact sheets showed: layout PSNR ranks the
UCF-trained arm above mixed above domain-matched (15.34 $\pm$ 0.02 / 14.19 / 13.57 $\pm$ 0.30
across training seeds, a six-standard-deviation structure gap) --- the exact reverse of LPIPS
--- so the battery now reports texture and structure separately. A matched
steps-per-view retrain worsens layout further, ruling out overexposure; a mixed 24-source stack
interpolates between the pure corpora on every axis, showing the two supervision sources are
complementary: teacher-domain fits supply feature and texture accuracy, real-video fields
supply macro-structure. Every generated arm still sits far below the trivial baseline on layout
(15.7 versus 22.6 at best), making the structure crossover --- not the texture crossover ---
the milestone the scaling program gates on per corpus doubling.
...
The convolutional mixer is not the only way to couple the set. Replacing it with an SSOG-style
attention field~\citep{pisoni2026ssog} --- per-atom Gaussians over relative cell displacement,
content steering through bounded cold-started residuals, applied separably with occupancy
riding the same three 1D passes so the field averages over born cells only --- reproduces the
exact-count gains at 66k added parameters instead of 414k: foreground PSNR +0.775 versus the
convolution's +0.761, edge/motion/quiet all improved, PSNR +0.224, with the same family of
failure (SSIM regresses; not selected). Its held-out screen gain is 2.4\(\times\) the
convolution's on a matched same-device battery, and the learned geometry is directly readable:
three of four atoms look backward in time with temporal reach wider than spatial ---
self-discovered temporal-context gathering --- and all three cold-started steering gates open
during training, the width gate hardest. One methods caveat these comparisons surfaced:
seeded evaluation noise paths differ across GPU architectures, offsetting absolute battery
values by more than typical model deltas, so all screens here are same-device.
...
\subsection{A feed-forward encoder replaces mark-space emission}

The accumulated evidence --- layout below the trivial-decode baseline for every sampler, a
data curve that moved texture but never structure, and coupling gains an order of magnitude
too small --- identifies joint composition under additive rendering as the failure mode of
direct set generation, and a visual review of the rollouts made the verdict operational: the
mark-space path is retired as primary. The replacement inverts the architecture. A $\sim$5M
feed-forward encoder performs \emph{amortized fitting}: slots are seeded from the video itself
on a stratified per-cell lattice with widths and opacities calibrated so the initial field
renders the video softly at unity coverage, and a 3D-convolutional trunk predicts bounded
refinements, trained with the fitter's own stochastic-voxel render loss on teacher-generated
(video, fitted-field) pairs --- a supervised regression with manufacturable data. Seeding is
load-bearing: the identical model without it underfits at 12.2 dB.

\begin{table}[H]
\centering
\small
\caption{One-shot encoding of the four held-out clips at native 288\(\times\)192 (final step-8000
checkpoint; the mid-run step-3000 snapshot already scored 23.24 / 0.4693). The encoder is the first arm to beat the trivial
decode on every metric.}
\label{tab:encoder}
\begin{tabular}{@{}lrrrr@{}}
\toprule
...
\textbf{Encoded one-shot (single forward pass)} & \textbf{23.41} & \textbf{0.9436} & \textbf{0.4049} & \textbf{30.14} \\
Guide upsample baseline & 19.35 & 0.8596 & 0.6762 & 22.61 \\
Best mark-space generative arm & 14.48 & 0.6031 & 0.6911 & 15.75 \\
\bottomrule
\end{tabular}
\end{table}

Macro-layout reaches the fitted ceiling --- the deficit that defined every generative arm is
eliminated --- and the remaining 4.3 dB is uniform softness with three supervised levers:
corpus (twelve windows here; the teacher mints more at $\sim$30 GPU-minutes each), capacity,
and refinement passes. The system prompt \(\rightarrow\) teacher video \(\rightarrow\)
encoder \(\rightarrow\) persistent jewel field is thereby end-to-end real: semantics at
teacher quality, persistence, identity, and editability at the field level. Set-space
emission continues as a research lane in latent-set form, with the steered Gaussian coupling
of the previous subsection as its substrate.
