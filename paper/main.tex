...
validates privileged scaffold-to-jewel transfer, not free prompt-only jewel generation.
...
LTX videos without mark-model retraining. We additionally articulate two falsifiable systems
...
semantics; those semantics arrive through the prompt-generated raster scaffold. The architectural
decision is now to learn scaffold-conditioned occupied cells, counts, and birth ranks while exact
stable-ID overlap state persists. Direct text-to-jewel distillation becomes sensible only after
that target-topology dependency is removed on a larger paired scaffold/jewel corpus.
...
\begin{itemize}
  \item \textbf{No autonomous text-to-jewel result.} The hybrid prompt-to-LTX-to-jewel path
  preserves three of four held-out actions, but target topology and the LTX future video remain
  privileged. The jewel model's correct and shuffled text controls are effectively tied.
  ...
  \item \textbf{Privileged semantic guide and topology.} The successful \(24\times40\) guide is
  decoded from a prompt-generated LTX future, but the fitted target still supplies birth cells and
  ranks. The transfer result proves scaffold-conditioned marks, not autonomous field construction.
...
\begin{enumerate}
  \item freeze the passing UCF-trained raster-guided realizer as the cross-domain baseline and
  expand paired LTX-scaffold/jewel coverage with held-out prompts and classes;
  \item learn scaffold-conditioned occupied-cell probability, positive counts, and birth ranks,
  removing fitted target topology from inference;
  \item generate overlapping windows by copying stable-ID jewels exactly and emitting only frontier
  births, with explicit density, seam, identity, and action-preservation gates;
  \item redesign differentiable rendering around foreground/motion-aware samples and explicit
  temporal stability only if the mark realizer, rather than topology, remains the limiting error;
...
claim. Its next milestone is no longer ``test whether the realizer transfers'' or ``add more local
tokens.'' It is to remove privileged target topology by predicting scaffold-conditioned occupancy,
counts, and birth ranks under exact overlap carry. This hybrid preserves the distinctive explicit
...
Mark generator & 22-D oracle-topology rectified flow with cell/rank queries and nullable text \\
Semantic control & prompt-generated LTX \(24\times40\) guide passes transfer; target topology remains privileged \\
