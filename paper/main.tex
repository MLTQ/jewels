...
monotone and unsaturated in held-out loss and LPIPS; training on teacher-generated fits removes
a 33\% cross-domain feature-loss tax; and a mixed real-plus-teacher corpus shows the two
...
four evaluation clips, and a correct-versus-shuffled margin that thins at twelve sources. A
first repeated-seed measurement calibrates the noise floor: retraining the 12-source point at a
second seed shifts held-out velocity loss by 0.075 --- the same order as the curve's
per-4-source improvements --- so the slope requires full multi-seed error bars before it can be
quoted with intervals, while the 0.51 domain-matching gap below is seven times this spread and
stands regardless.
...
UCF-trained arm above mixed above domain-matched (15.33 / 14.19 / 13.38) --- the exact reverse
of LPIPS --- so the battery now reports texture and structure separately. A matched
