...
near-photoreal perceptual quality. Second, the generative stack is real but immature: from an
empty field, a 0.88M topology head and a 2.13M rectified-flow realizer generate three 16-frame
windows with exact append-only identity, beating shuffled-scaffold controls by 2.8--4.1 dB, yet
a no-model trilinear decode of the stack's own scaffold input still wins every reference-based
fidelity metric --- so we triangulate with perceptual (LPIPS) and texture-blind layout metrics,
which disagree with PSNR and with each other in instructive ways. Third, quality scales with
