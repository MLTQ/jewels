...
retrieval gate. Current trajectory macros are source-backed. The next experiment is to learn
reusable, source-independent trajectory prototypes at larger data and compute scale.
\end{abstract}
...
The experiments separate representation capacity, semantic preservation, and native generation.
Reconstructing an existing video shows that the representation can hold video, not that language
can generate it. A prompt-conditioned scaffold followed by an encoder tests semantic preservation,
not native prompt-to-Jewel generation. Retrieval of a whole fitted source field is also not
generation. We use input-ownership audits and shuffled/null controls to isolate the prompt-only
claim.
...
does reuse source-backed trajectory programs. The next gate replaces those macros with reusable,
source-independent prototypes.

\begin{figure*}[t]
  \centering
  \resizebox{0.96\textwidth}{!}{\input{figures/architecture_overview.tex}}
  ...
  foreground/background ownership, addressed phrases, and physical tokens. Current macro tokens
  are backed by 18 fitted sources.}
...
\begin{itemize}
  ...
  \item causal and scaling evidence that temporal tilt improves reconstruction and that amortized
  video-to-Jewel prediction improves with data rather than collapsing to a coordinate grid;
...
Here, output tokens expand into persistent local operations in displayed spacetime. We test whether
a hierarchical, executable Gaussian language has the required representational and semantic
properties.
...
\begin{figure}[t]
  \centering
  \resizebox{\linewidth}{!}{\input{figures/jewel_geometry.tex}}
...
Centroid placement is audited separately. Centroids are continuous and jittered; local phrases do
not place one primitive on each fixed cell. We report the fraction of centers locked to the implied
grid and require zero. This permits structured addressing without visible coordinate quantization.
...
In the tested prompt-only systems, trajectory-level ownership carried semantic structure. A global
scene decoder and a purely addressed $16\times16\times8$ local phrase model each produced zero
recognizable classes, although the local model improved token NLL from 6.123 to 5.168. Holding one
coherent source through the
...
Space--time covariance improves the tested reconstructions. \\
...
\subsection{Temporal tilt improves reconstruction}
...
is 27.699 dB, LPIPS 0.1720, and median tilt 0.3966. Relative to the 120-example encoder, the measured
gaps are 5.071 dB, 0.2196 LPIPS, and 0.2091 median tilt.
...
\begin{figure}[t]
  ...
  95\% $t$ interval over three seeds. Right: convergence against matched corpus exposure. The
  result is monotonic across these three small training sets.}
  \label{fig:scaling}
\end{figure}
...
91.4\% of the source video's prompt alignment. We exclude this result from the native-generation
claim because a pretrained video model supplies the source video.
...
\begin{figure}[t]
  ...
  \end{minipage}
  \caption{Prompt-only evidence sheet reflowed from one audited image. Each half shows correct-prompt
...
\subsection{Learned speaker results}
...
The strict rendered top-1 test passes only 4/9, below the declared 6/9 threshold, and only one of
three class majorities passes. The network learned the coarse scene syntax under unseen wording but
did not reliably choose donor combinations whose low-fidelity renders win a frozen semantic
retrieval test. Held-out correct NLL plateaued early, so the next experiment changes the vocabulary
and supervision rather than extending the same training run: learn reusable trajectory prototypes
...
\section{Scaling priorities}
\label{sec:scaling}

The experiments isolate three different scaling questions.

\paragraph{Representation compute.}
Longer local appearance optimization improves the frozen-geometry derivative head. For
...
bit-identical. The plateau after roughly 8--12k updates limits the expected gain from extending the
same optimization run.

\paragraph{Encoder data.}
Held-out PSNR rises at each data scale and remains below the fitted teacher. The current 120-example
maximum is far too small for a foundation model claim. A follow-up study should fix architecture
and corpus passes, increase data by at least another order of magnitude, and measure perceptual
fidelity, layout, mixed tilt, and prompt retention jointly.

\paragraph{Generative vocabulary.}
We propose segmenting many fitted fields into persistent trajectory tubes using motion and semantic
features, learning a dictionary whose
entries aggregate across source videos, and holding out entire motions, appearances, and
compositions. A speaker should emit
prototype identities plus continuous transforms and residuals; a local flow or diffusion head can
model residual 22-D marks \cite{lipman2023flow,ludke2024pointset,hui2025notsoptimal}. The proposed
gate requires prompt-only renders whose macro tokens are source-independent, whose prompts and
motions are held out compositionally, and whose correct/shuffled/null margins persist over many
classes and seeds.
...
system has not learned source-independent motion concepts. This prevents an open-vocabulary claim.
...
The measurements support a specific proposition: persistent time-tilted Gaussian Jewels improve
reconstruction under matched resources, their amortized prediction improves with data, their
physical parameters admit an irregular hierarchical tokenization, and prompt plus seed can drive a
bounded native program with controlled semantic selectivity. They do not support the claim that we
have a general text-to-video model. The next step is to replace source-backed trajectory macros with
reusable learned prototypes and test compositional prompt-only generation on held-out motions and
appearances. That experiment tests whether the current result scales with additional data and
compute.
