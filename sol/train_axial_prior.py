"""Train a text-conditioned axial flow on hierarchical block latents."""
...
from sol.latent_prior import flow_matching_loss
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--eval-sample-steps", type=int, default=15)
...
def main() -> None:
    ...
    print(
        ...
        f"latent={tuple(train_latents.shape[1:])} grid={model.grid_shape} "
        f"condition={train_conditions.shape[1]} model={parameters / 1e6:.2f}M amp={use_amp}",
        flush=True,
    )
