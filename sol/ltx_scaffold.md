...
- **Does**: Pins the selected CUDA device and expandable allocator, launches LTX, probes the MP4,
  and records success or failure provenance.
...
| Research audit | JSON receipt includes prompt, seed, command, revision, runtime, and probe | Receipt schema |
...
- Required upstream revision: `4f8905737aac86a554637cac86c178877a39c744` until deliberately
  upgraded.
- LTX's 46.15 GB distilled checkpoint and 1.0 GB upscaler are installed. The 24.38 GB Gemma encoder
  is gated by Google's usage license and must be accepted by the account owner before download.
...
After the `Multiqueue` Hugging Face account has accepted the Gemma license and the encoder has been
downloaded, run the first bounded scaffold from `/home/m/jewels`:
...
The command writes both the MP4 and a neighboring JSON receipt. `--dry-run` validates geometry and
prints the exact argument vector without requiring model assets or starting CUDA work.
