...
class BirthContinuationModel(nn.Module):
    ...
    def cell_states(self, context: torch.Tensor) -> torch.Tensor:
        if context.ndim == 1:
    ...
    def forward_training(
        self, context_raster: torch.Tensor, target: BirthTarget
    ) -> BirthModelOutput:
        context = self.encode_context(context_raster)
        return self.forward_from_context(context, target)

    def forward_from_context(
        self, context: torch.Tensor, target: BirthTarget
    ) -> BirthModelOutput:
        """Evaluate target ranks from a precomputed or ablated context embedding."""
        cells = self.cell_states(context)
    ...
    @torch.no_grad()
    def decode(self, context_raster: torch.Tensor) -> DecodedBirths:
        context = self.encode_context(context_raster)
        cells = self.cell_states(context)
