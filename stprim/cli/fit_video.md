...
- Saves full `state_dict` + cfg + info per run. `info["background"]` is part of the model —
  see fitter.md.
- Exposes `--cull-mode` and all finite-support budget/index controls. This keeps the fast historical
  KNN baseline available while allowing a checkpoint to record either the all-center support oracle
  or support-complete tiled candidates. `--cull-mode exact` is intentionally limited to tiny audits.
